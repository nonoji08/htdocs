...
<div id="wrap" class="sub_bg">
    ...
    <div id="header">	
		...
		<div id="site_gnb">	
			<ul class="gnb_menu">
			<li><a href="#">로그인</a></li>
			<li><a href="register/kcc_copy.php">회원가입</a></li>
			<li><a href="#">사이트맵</a></li>
			</ul>
			<a class="m_gnb_ico"></a>
		</div>
		<div id="site_menu">
			...
			<div class="menu_lst_wrap">
				...
				<li class="menu5">
					...
					<ul class="site_sub_menu">
					<li><a href="notice.php" class="sp_com site_menu5_1">공지사항</a></li>
	...
	<div id="container">
		<div id="content">
			...
			<div class="left_con">
				...
				<ul class="lnb">
									<li><a href="notice.php" class="selected_hard">공지사항<span class="sp_sub select_ico"></span></a></li>
				...
				<form id='writeBoard' action='notice_update.php' method='post'>
				<?php
					include 'include.php';
					$idx=$_REQUEST['idx'];
					$sql = "SELECT * FROM kcc_board WHERE idx=$idx";
					...
					. "$('#title').val('$title'); $('#name').val('$name'); $('#idx').val('$idx');"
					. "});</script>";
				?>
				<div class="article_box">
					<table class="not_wrt">
						<th class='write_caption'>제목</th>
						<td colspan='3' class='tb_title'><input type='text' class="inptxt wsubject" id="title" name='title'/></td>
						</tr></thead>
						<tbody>
						<th class='write_caption'>작성자</th> 
						<td colspan='3' class='tb_title'><input type='text' class="inptxt w70" readonly id="name" name='name'/><span class='write_alert'>작성자는 수정이 불가능합니다.</span></td>
						<tr><th class='write_caption'>첨부파일</th>
							<td id='boardFile' colspan='3'>
								<?=$file_name?><span class='write_alert'>파일은 수정이나 삭제가 불가능합니다.</span>
							</td>
							</tr>
					...
					<div class='wrapBtnview'>
						...
						<a href="notice.php" class="sp_coms btn_view">취소</a>
					</div>
				</div>
			</form>
					</div>
