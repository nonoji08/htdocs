		...
		<div id="content">
			...
			<div class="left_con">
				...
				<?php
					...
					$hit = $row['hit'];
					$hit_up = $hit+1;
					$hitsql = "update kcc_board set hit=$hit_up where idx=$idx";
					$stmt = $pdo->prepare($hitsql);
					$stmt->execute();
					
					echo "
					<tr class='view_tr'>
						<td>제목:$title</td><td>작성일:$date</td><td>조회수:$hit</td>
					</tr>
					<tr class='view_tr'><td class='view_con' colspan=3>$content</td></tr>";
					if($file_name) echo "<tr><td id='boardFile' colspan='3'>첨부파일:<a href='file_download.php?idx=$idx'>$file_name</a></td></tr>";
				?>
				</table>
						<div class='wrapBtnview'>
							<?php
								echo"<a href='notice_edit.php?idx=$idx' class='sp_coms btn_view'>수정하기</a>"
									. "<a href='notice_delete.php?idx=$idx' class='sp_coms btn_view'>삭제하기</a>";
							?>
